...
from steadylab.msg import ErpRead
from steadylab.msg import ErpWrite
...
class Serial(Node):  # Node 클래스를 상속받습니다.
    def __init__(self):
        ...
        self.sub_speed_steer = self.create_subscription(ErpWrite, 'serial', self.callback, qos)
        self.erp_sub = self.create_subscription(ErpRead, 'erp_read', self.callback, qos)

        self.erp_pub = self.create_publisher(ErpWrite, 'erp_write', qos)
        self.erp = ErpWrite()
